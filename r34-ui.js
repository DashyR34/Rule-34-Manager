...
(function () {
    ...
    /* Top-level helpers (moved here so they are available) */
    ...
    function applyAriaLabels(root = document.body) {
    ...
    function injectPreloadStyles() {
        const anim = getConfig("animSpeed", 0.75);
        const style = document.createElement("style");
        style.id = "r34-preload-style";
        style.textContent = `.thumb { opacity: 0 !important; transition: opacity ${anim}s cubic-bezier(0.34, 1.56, 0.64, 1); }`;
        (document.head || document.documentElement).appendChild(style);
    }
    function removePreloadStyles() {
        const style = document.getElementById("r34-preload-style");
        if (style) style.remove();
        document
            .querySelectorAll(".thumb")
            .forEach((thumb) => (thumb.style.opacity = "1"));
    }
    ...
    class UIManager {
        constructor(filterManager, blacklistManager) {
            ...
            this._activeDisplay = { /* your original object */ };
	...
	table.highlightable td {
        ...
        /* ==================== BOUNCY MEDIA PREVIEW (IMG + VIDEO) - FULLY POLISHED ==================== */
